...
class Canvas(QtWidgets.QWidget):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Menus:  右键菜单
        # 0: right-click without selection and dragging of shapes,用于在画布空白处右键点击时的通用菜单
        # 1: right-click with selection and dragging of shapes,用于选中图形后，按住Ctrl再右键点击时的特定菜单 (移动/复制到此处)
        ...
        # --- 旋转状态变量 ---
        self._isRotating = False  # 是否正在旋转
        self._rotatingShape = None  # 正在旋转的 Shape 对象
        self._rotationCenter = None  # 旋转中心点
        self._rotationStartPoint = None  # 鼠标开始拖动的点 (用于计算角度)
        self._initialPoints = None  # 旋转开始时，图形的原始顶点列表
        ...
        if ev.button() == QtCore.Qt.LeftButton:  # type: ignore[attr-defined]
            ...
            if self.drawing():
                ...
                if self.createMode in ["pen","eraser"]:
                    ...
                    if self.createMode == "eraser":
                        ...
                        self.current_brush.last_point = pos
                        Brush.PEN_WIDTH = self.eraser_width
                    elif self.createMode == "pen":
                        # 根据当前选中的标签和颜色，创建一个画笔笔刷
                        self.current_brush = Brush(self.current_label, self.label_color, brush_type='pen',)
                        self.current_brush.last_point = pos
                        Brush.PEN_WIDTH=self.pen_width  # 设置画笔宽度
            ...
            # --- 分支 B：如果当前处于“编辑模式” ---
                ...
                if self.selectedEdge() and ev.modifiers() == QtCore.Qt.AltModifier:  # type: ignore[attr-defined]
                    self.addPointToEdge()
                elif self.selectedEdge() and ev.modifiers() == QtCore.Qt.ControlModifier:
                    self._isRotating = True
                    self._rotatingShape = self.prevhShape
                    self._rotationCenter = self._rotatingShape.get_centroid()
                    print(self._rotationCenter)  # 中心点
                    if self._rotationCenter is None:  # 检查中心点计算
                        self._isRotating = False
                        print("无法为多边形")
                        logger.warning(f"无法为多边形 '{self._rotatingShape.label}' 计算中心点，无法旋转。")
                    else:
                        self._rotationStartPoint = pos
                        self._initialPoints = [p for p in self._rotatingShape.points]  # 备份初始点
                        # self.setToolTip(self.tr("Release to finalize rotation"))
                        print("Release to finalize rotation")
                        self.overrideCursor(CURSOR_MOVE)  # 设置光标
                        self.update()  # 更新界面
                        # !!! 关键：旋转事件已处理，直接返回 !!!
                        return
    ...
    def canCloseShape(self):
        return self.drawing() and (
            ...
            or self.createMode in ["ai_无法为多边形", "ai_mask"]
        )
    ...
    def paintEvent(self, event: Optional[QtGui.QPaintEvent]) -> None:
        ...
        if (
            ...
            # 3. [核心修正] 计算半径
            # 不再除以 self.scale。
            # base_size 是图像上的像素大小 (例如 20)。
            # 由于当前 p 已经 scale 过了，绘制半径 10 的圆，
            # 在 2倍 缩放下，屏幕上就会显示 直径 40 的圆。这正是你要的效果。
...
#检查当前鼠标位置 pos 是否超出图像范围
    ...
    def wheelEvent(self, ev):
        ...
        # [核心修复]
        # 只要是画笔/橡皮擦模式，无论滚轮做了什么操作（包括调整笔刷粗细），
        # 都强制刷新界面。这样光标大小的变化就是丝滑实时的，不需要移动鼠标来触发。
        if self.createMode in ["pen", "eraser"]:
            self.update()
