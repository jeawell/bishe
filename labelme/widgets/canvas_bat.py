...
class Canvas(QtWidgets.QWidget):
    ...
    def mouseMoveEvent(self, ev):
        ...
        if self.drawing():
            ...
            if self.createMode in ["polygon","linestrip","line"]:
                ...
                if self.mousePress == True and self.mouseRealse == False:
                    self.eraser_pos = pos
                    print(self.eraser_pos)
    ...
    def mouseReleaseEvent(self, ev):
        ...
        if self.createMode in ['brush'] and self.current is not None:
            self.finalise()
    ...
    def paintEvent(self, event: Optional[QtGui.QPaintEvent]) -> None:
        ...
        # if (self.createMode=='eraser' and self.drawing()
        #         and not self.outOfPixmap(self.prevMovePoint)
        # ):
        #     print("quan")
        #     p.setPen(QPen(Qt.red, 10, Qt.DashLine))
        #     p.setBrush(Qt.NoBrush)
        #     p.drawEllipse(self.eraser_pos, self.eraser_radius, self.eraser_radius)
        ...
        for shape in self.shapes:
            if (shape.selected or not self._hideBackround) and self.isVisible(shape):
                ...
                shape.paint(p)
            if self.createMode=='eraser':
               if self.pathNear(shape.points,self.eraser_pos,self.eraser_radius):
                   self.contact_del_shape.emit([shape])
        if self.current:
